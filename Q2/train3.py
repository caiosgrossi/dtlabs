...
from torchvision.models import resnet50, ResNet50_Weights  # SE-ResNet50 usa a mesma API
import pandas as pd
import numpy as np
...
# Definição do modelo de extração de embeddings com SE-ResNet50
class FaceEmbeddingModel(nn.Module):
    def __init__(self, embedding_dim=128):
        super(FaceEmbeddingModel, self).__init__()
        # Utiliza SE-ResNet50 pré-treinado
...
torch.save(model.state_dict(), "face_embedding_model_seresnet50_3.pth")
print("Modelo salvo como face_embedding_model_seresnet50_3.pth")
...
df.to_csv('celebridades_embeddings_seresnet50_2.csv', index=False)
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Loss de Treino e Validação por Epoch (SE-ResNet50)")
plt.legend()
plt.grid(True)
plt.savefig(f"loss_plot_epochs_{num_epochs}_batch_{batch_size}_seresnet50.png")
plt.close()
